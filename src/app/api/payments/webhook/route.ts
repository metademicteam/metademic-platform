    ...
    if (status === "succeeded" || status === "paid" || status === "success") {
      ...
      if (mid) {
        ...
        if (authorId) {
          ...
          await admin.from("email_logs").insert({ user_id: authorId, manuscript_id: mid, recipient_email: "author", template_name: "payment_received", subject: "Payment received — Metademic", status: "queued" } as never);
        }
      }
